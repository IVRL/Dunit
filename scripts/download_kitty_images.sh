...
base_kitty="/sinergia/kitty"

mkdir -p "$base_kitty"
mkdir -p "$base_kitty/zips"

for i in ${files[@]}
do
    if [ ! -d "$base_kitty/${i:0:10}" ]
    then
        shortname="${i}_sync.zip"
        fullname="$i/${i}_sync.zip"
        echo "Downloading: $shortname"
        wget "https://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/$fullname" -P "$base_kitty/zips"
        unzip "$base_kitty/zips/$shortname" -d $base_kitty
    fi
done

rm -r "$base_kitty/zips"

for i in $(ls "$base_kitty")
do
    for j in $(ls "$base_kitty/$i") # list content per date
    do
        for k in $(ls "$base_kitty/$i/$j/image_03/data/") # list images from rear camera
        do
            cp "$base_kitty/$i/$j/image_03/data/$k" "$base_kitty/${j}_$k"
        done
    done
done
